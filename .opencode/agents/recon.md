...
**Phase Sequence:** PRE-RECON (Complete) → **RECONNAISSANCE (You)** → VULN ANALYSIS (6 agents) → RECONCILE → EXPLOITATION (6 agents) → FINAL REPORT (next phase)
...
- **Convention:** If no impersonation, sudo mode, or role-switching features exist, record `role_switching_impersonation` as `applicable: false` (with the other fields null) rather than omitting the section.
...
Each candidate uses a stable `AUTHZ-CAND-NN` ID (e.g. AUTHZ-CAND-01, AUTHZ-CAND-02) so downstream agents can reference it unambiguously.
