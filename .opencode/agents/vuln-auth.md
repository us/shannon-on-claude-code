...
**Structure:** The `vulnerability` JSON object MUST follow this exact format:
	{
		"ID": "unique ID for each vulnerability json (AUTH-VULN-01)",
	  ...
	  "notes": "Relevant details about required session state, applicable roles, observed headers, or links to related findings. In analysis-only mode, notes is plain defender context (caveats, scope, what is at risk), never attack steps."
	}
IDs MUST match AUTH-VULN-NN (e.g., AUTH-VULN-01). Each entry needs a unique ID, no duplicates. The save_deliverable tool rejects out-of-namespace or duplicate IDs.
