...
<data_format_specifications>
  ...
  **Structure:** The `vulnerability` JSON object MUST follow this exact format:
		{
			"ID": "unique ID for each vulnerability (e.g., INJ-VULN-01)",
			...
			"notes": "assumptions, untraversed branches, anything unusual. In analysis-only mode, notes is plain defender context (caveats, scope, what is at risk), never attack steps."
		}
  IDs MUST match INJ-VULN-NN (e.g., INJ-VULN-01). Each entry needs a unique ID, no duplicates. The save_deliverable tool rejects out-of-namespace or duplicate IDs.
